...
const createSocketServer = (server) => {
  ...
  io.on("connection", (socket) => {
    ...
    redisSubscriber.subscribe("notification", (message) => {
      try {
        ...
        if (notification && notification.userId) {
          io.to(notification.reciveId).emit(
            "receiveNotification",
            notification
          );
          console.log(
    ...
    socket.on("joinGroup", async ({ userId, groupId }) => {
      if (!userId || !groupId) {
        console.error(
          "joinGroup event received with missing userId or groupId"
        );
        return;
      }
      try {
    ...
    socket.on("sendMessage", async (data) => {
      try {
        ...
        if (!idConversation || !message) {
          console.error(
            "Missing idConversation or message in sendMessage event"
          );
          return;
        }
    ...
    socket.on("leaveGroup", async ({ userId, groupId }) => {
      if (!userId || !groupId) {
        console.error(
          "leaveGroup event received with missing userId or groupId"
        );
        return;
      }
      try {
